...
// Contract obligation tracker with alerts for renewals/SLA breaches
// Feature: contract-obligation-tracker
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Contract obligation tracker with alerts for renewals/SLA breaches.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'contract-obligation-tracker', payload, result);
    res.json({ success: true, feature: 'contract-obligation-tracker', ...result });
  } catch (err) {
    console.error('contract-obligation-tracker analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Contract obligation tracker with alerts for renewals/SLA breaches. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'contract-obligation-tracker', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'contract-obligation-tracker']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'contract-obligation-tracker', title: 'Contract obligation tracker with alerts for renewals/SLA breaches', project: 'AIStrategicSourcingNegotiation' });
});
