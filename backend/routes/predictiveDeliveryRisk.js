...
// Predictive delivery risk flagging suppliers likely to miss dates
// Feature: predictive-delivery-risk
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Predictive delivery risk flagging suppliers likely to miss dates.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'predictive-delivery-risk', payload, result);
    res.json({ success: true, feature: 'predictive-delivery-risk', ...result });
  } catch (err) {
    console.error('predictive-delivery-risk analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Predictive delivery risk flagging suppliers likely to miss dates. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'predictive-delivery-risk', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'predictive-delivery-risk', title: 'Predictive delivery risk flagging suppliers likely to miss dates', project: 'AIStrategicSourcingNegotiation' });
});
