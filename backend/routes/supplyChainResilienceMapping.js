...
// Supply chain resilience mapping identifying single-sourced categories
// Feature: supply-chain-resilience-mapping
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIStrategicSourcingNegotiation" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Supply chain resilience mapping identifying single-sourced categories.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'supply-chain-resilience-mapping', payload, result);
    res.json({ success: true, feature: 'supply-chain-resilience-mapping', ...result });
  } catch (err) {
    console.error('supply-chain-resilience-mapping analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIStrategicSourcingNegotiation". Return strict JSON only.',
      `Feature: Supply chain resilience mapping identifying single-sourced categories. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'supply-chain-resilience-mapping', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'supply-chain-resilience-mapping']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'supply-chain-resilience-mapping', title: 'Supply chain resilience mapping identifying single-sourced categories', project: 'AIStrategicSourcingNegotiation' });
});
