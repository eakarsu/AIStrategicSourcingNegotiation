...
export default function CfContractObligationTrackerWithAlertsForRenewals() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-contract-obligation-tracker-with-alerts-for-renewals-sla/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-contract-obligation-tracker-with-alerts-for-renewals-sla', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-contract-obligation-tracker-with-alerts-for-renewals-sla/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Contract obligation tracker with alerts for renewals/SLA breaches</h1>
