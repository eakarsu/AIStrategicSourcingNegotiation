...
export default function CfInvoiceAnomalyDetectionForManualReview() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-invoice-anomaly-detection-for-manual-review/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-invoice-anomaly-detection-for-manual-review', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-invoice-anomaly-detection-for-manual-review/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Invoice anomaly detection for manual review</h1>
