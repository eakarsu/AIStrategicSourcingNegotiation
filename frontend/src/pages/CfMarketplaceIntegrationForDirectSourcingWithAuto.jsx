...
export default function CfMarketplaceIntegrationForDirectSourcingWithAuto() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-marketplace-integration-for-direct-sourcing-with-auto-price-monitoring/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-marketplace-integration-for-direct-sourcing-with-auto-price-monitoring', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-marketplace-integration-for-direct-sourcing-with-auto-price-monitoring/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Marketplace integration for direct sourcing with auto-price monitoring</h1>
