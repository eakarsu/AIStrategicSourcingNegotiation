...
export default function CfPredictiveDeliveryRiskFlaggingSuppliersLikelyTo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-delivery-risk-flagging-suppliers-likely-to-miss/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-predictive-delivery-risk-flagging-suppliers-likely-to-miss', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-delivery-risk-flagging-suppliers-likely-to-miss/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Predictive delivery risk flagging suppliers likely to miss dates</h1>
