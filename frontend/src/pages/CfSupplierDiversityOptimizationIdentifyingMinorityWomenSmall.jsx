...
// Feature: Supplier diversity optimization identifying minority/women/small suppliers with AI scoring
...
export default function CfSupplierDiversityOptimizationIdentifyingMinorityWomenSmall() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-supplier-diversity-optimization-identifying-minority-women-small-suppliers/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-supplier-diversity-optimization-identifying-minority-women-small-suppliers', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-supplier-diversity-optimization-identifying-minority-women-small-suppliers/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Supplier diversity optimization identifying minority/women/small suppliers with AI scoring</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIStrategicSourcingNegotiation</p>
      </div>
