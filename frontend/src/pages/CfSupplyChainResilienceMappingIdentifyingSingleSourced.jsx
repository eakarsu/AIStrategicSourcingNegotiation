...
export default function CfSupplyChainResilienceMappingIdentifyingSingleSourced() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-supply-chain-resilience-mapping-identifying-single-sourced-categories/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-supply-chain-resilience-mapping-identifying-single-sourced-categories', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-supply-chain-resilience-mapping-identifying-single-sourced-categories/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Supply chain resilience mapping identifying single-sourced categories</h1>
