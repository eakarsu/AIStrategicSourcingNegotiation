...
export default function GapLimitedIntegrationsOnlyAnExportModuleNo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integrations-only-an-export-module-no-erp/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-integrations-only-an-export-module-no-erp', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-integrations-only-an-export-module-no-erp/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited integrations (only an export module — no ERP, SAP, Oracle, Coupa)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AIStrategicSourcingNegotiation</p>
      </div>
