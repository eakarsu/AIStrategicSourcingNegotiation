...
export default function GapNoAiDrivenSupplierDiversityOptimization() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-supplier-diversity-optimization/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-ai-driven-supplier-diversity-optimization', project: 'AIStrategicSourcingNegotiation' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-ai-driven-supplier-diversity-optimization/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No AI-driven supplier diversity optimization</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIStrategicSourcingNegotiation</p>
      </div>
